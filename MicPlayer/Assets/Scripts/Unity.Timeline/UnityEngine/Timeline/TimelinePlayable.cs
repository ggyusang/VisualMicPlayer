...
namespace UnityEngine.Timeline
{
	public class TimelinePlayable : PlayableBehaviour
	{
		...
		private Playable CreateTrackPlayable(PlayableGraph graph, Playable timelinePlayable, TrackAsset track, GameObject go, bool createOutputs)
		{
			...
			Playable value;
			if (m_PlayableCache.TryGetValue(track, out value))
			{
				return value;
			}
			if (track.name == "root")
			...
			if (!playable2.IsValid())
			{
				...
				throw new InvalidOperationException(name + "(" + (((object)type != null) ? type.ToString() : null) + ") did not produce a valid playable. Use the compilable property to indicate whether the track is valid for processing");
			}
