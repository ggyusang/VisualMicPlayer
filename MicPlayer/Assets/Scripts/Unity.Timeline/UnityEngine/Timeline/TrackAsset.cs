...
namespace UnityEngine.Timeline
{
	...
	public abstract class TrackAsset : PlayableAsset, ISerializationCallbackReceiver, IPropertyPreview, ICurvesOwner
	{
		...
		public override IEnumerable<PlayableBinding> outputs
		{
			get
			{
				TrackBindingTypeAttribute value;
				if (!s_TrackBindingTypeAttributeCache.TryGetValue(GetType(), out value))
				{
		...
		public void CreateCurves(string curvesClipName)
		{
			if (!(m_Curves != null))
			{
				m_Curves = TimelineCreateUtilities.CreateAnimationClipForTrack(string.IsNullOrEmpty(curvesClipName) ? "Track Parameters" : curvesClipName, this, true);
			}
		}
		...
		{
			object[] customAttributes = GetType().GetCustomAttributes(typeof(TrackClipTypeAttribute), true);
			Type type = null;
			...
			{
				Type type2 = GetType();
				Debug.LogWarning("Cannot create a default clip for type " + (((object)type2 != null) ? type2.ToString() : null));
				return null;
			}
		...
		internal TimelineClip CreateClip(Type requestedType)
		{
			...
			string obj = (((object)requestedType != null) ? requestedType.ToString() : null);
			Type type = GetType();
			throw new InvalidOperationException("Clips of type " + obj + " are not permitted on tracks of type " + (((object)type != null) ? type.ToString() : null));
		}
		...
		internal TimelineClip CreateClipFromPlayableAsset(IPlayableAsset asset)
		{
			...
			if (!ValidateClipType(asset.GetType()))
			{
				...
				Type type2 = GetType();
				throw new InvalidOperationException("Clips of type " + obj + " are not permitted on tracks of type " + (((object)type2 != null) ? type2.ToString() : null));
			}
		...
		private void UpdateDuration()
		{
			...
			if (num != m_ItemsHash)
			{
				...
				double outStart;
				double outDuration;
				GetSequenceTime(out outStart, out outDuration);
